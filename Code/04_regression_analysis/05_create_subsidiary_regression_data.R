...
# (1) identify all entries from patents by U.S. subsidiaries of non-US companies
...
# (2) classifiy those that are onshored:
onshoring_p_keys <- setDT(tmp)[ctry_inv == "US",
                               .(onshored = ifelse(.N >= 1, 1, 0)), 
                               by = .(p_key)]
...
#### evaluate
paste(nrow(tmp), "patents used for analysis") 
# N = 202'959
...
# N = 39'940 without adjustment

# (8) merge with inventor information
inv <- read.csv(paste0(getwd(), "/Data/regression_data/regression_data.csv"))
test <- names(inv)
inv <- inv %>% select(-onshored_patents)
patent_dat <- merge(patent_dat, inv, by = c("regio_inv", "TechGroup", "TimePeriod"), all.x = TRUE)
...
# write.csv(patent_dat, paste0(getwd(), "/Data/regression_data/regression_data_subsidiaries.csv"), 
#           row.names = FALSE)
# print("Saved dataset as 'regression_data_subsidiaries.csv'")
