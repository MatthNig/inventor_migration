##################################################################
# Description:  Exclude onshored patents from firms located in   #
#               tax havens.                                      #
...
# (3) Drop all those that are from tax haven countries as defined in Zucman:
...
# N = 130'427 (N = 165'532 in baseline). Difference = 35'105
...
patent_dat <- patent_dat %>% 
  rename(onshored_patents_no_taxhavens = onshored_patents) %>%
  select(regio_inv, TechGroup, TimePeriod, onshored_patents_no_taxhavens)
...
patent_dat <- mutate(patent_dat, 
                     onshored_patents_no_taxhavens = ifelse(is.na(onshored_patents_no_taxhavens), 
                                                            0, onshored_patents_no_taxhavens)
                     )

# test
if(identical(reg_dat, patent_dat[, !names(patent_dat) %in% "onshored_patents_no_taxhavens"])){
