...
dat <- country_onshoring(df = df, onshoring_country = "US", collaboration = FALSE,
                         triadic_only = FALSE, inventor_number = 1, 
                         world_class_indicator = TRUE) # only world class (i.e. top 10%)
...
# N = 28'650 (N = 165'532 in baseline). Difference = 136'882

##########################################################
####### Add to robustness check regression dataset #######
##########################################################

# (8) merge to the remaining data
test <- names(reg_dat)
patent_dat <- patent_dat %>% 
        dplyr::rename(onshored_patents_worldclass = onshored_patents) %>%
        dplyr::select(regio_inv, TechGroup, TimePeriod, onshored_patents_worldclass)
...
patent_dat <- mutate(patent_dat, 
                     onshored_patents_worldclass = ifelse(is.na(onshored_patents_worldclass), 
                                                      0, onshored_patents_worldclass)
)

# test
if(identical(reg_dat, patent_dat[, !names(patent_dat) %in% "onshored_patents_worldclass"])){
